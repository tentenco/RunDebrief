...
- English, Traditional Chinese, and Simplified Chinese app interfaces;
- Apache-2.0 source.
...
There is no official signed or notarized installer yet. The `v0.1.0-alpha` binary will be published separately only after Developer ID signing, Apple notarization, checksums, and clean-install QA pass.
...
目前沒有官方簽章或 Apple 公證的安裝檔。`v0.1.0-alpha` binary 會在 Developer ID 簽章、Apple 公證、checksums 與 clean-install QA 通過後另行發布。
...
目前没有官方签名或 Apple 公证的安装包。`v0.1.0-alpha` binary 会在 Developer ID 签名、Apple 公证、checksums 与 clean-install QA 通过后另行发布。
